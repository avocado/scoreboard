...
Score.Team = SC.Record.extend({
  ...
  points: function() {
    var points = 0;
    points += this.get('won') * Score.Match.POINTS_AWARDED['won'];
    points += this.get('draw') * Score.Match.POINTS_AWARDED['draw'];
    points += this.get('lost') * Score.Match.POINTS_AWARDED['lost'];
    return points;
  }.property('matches').cacheable()
...
Score.Group = SC.Record.extend({
  ...
  standings: function(key, value) {
    var q = SC.Query.local(Score.Team, {orderBy: 'points DESC, played ASC, goalDifference DESC, goalsFor DESC'});
